...
type ActiveTab = 'ingredients' | 'dishes' | 'combos' | 'calendar' | 'costs' | 'validation' | 'history';
...
export default function Home() {
  ...
  const tabs: { id: ActiveTab; label: string }[] = [
    ...
    { id: 'calendar', label: '04 Календарь' },
    { id: 'costs', label: '05 Аналитика, расходы и объемы' },
    { id: 'validation', label: `Проверка${validationIssues.length > 0 ? ` (${validationIssues.length})` : ''}` },
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 font-sans text-gray-900 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="no-scrollbar flex space-x-2 overflow-x-auto border-b border-gray-200">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`whitespace-nowrap border-b-2 px-4 py-3 text-sm font-medium transition-colors ${activeTab === tab.id ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              {tab.label}
            </button>
          ))}
